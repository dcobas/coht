...
typedef struct {
	uint32_t IsrFlag; /**< If 0 then no ISR is required */
	uint32_t Vector;
	uint32_t Level;
...
typedef struct {
	...
	uint32_t      SpaceNumber; /**< SpaceNumber for carrier boards its just
				      a number */
	uint32_t      DataWidth;
	uint32_t      WindowSize;
	InsLibEndian  Endian;
...
typedef struct {
	...
	uint32_t        DebugFlags;
	uint32_t        EmulationFlags;
	uint32_t        ModuleCount;    /**< how many modules driver controls */
