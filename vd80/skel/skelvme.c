 ...
 * @return 1 - on success
 */
static int map_vmeas(SkelDrvrModuleContext * mcon,
		     InsLibVmeAddressSpace * vas)
{
	...
	if (vas->Mapped == (void *) (-1)) {
		vas->Mapped = NULL;
		SK_ERROR
		    ("VME mapping failed while mapping 0x%x for module#%d",
...
static int AddVmeModule(SkelDrvrModuleContext * mcon)
{
	...
	if (intrd && intrd->IsrFlag) {
		cc = vme_intset(intrd->Vector, skel_isr, (char *) mcon,
				NULL);
		if (cc < 0) {

			SK_ERROR
			    ("module#%d: ISR not installed. retval = %d",
