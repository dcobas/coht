 ...
 * @return 1 - on success
 */
static int map_pcias(SkelDrvrModuleContext * mcon,
		     InsLibPciAddressSpace * pas)
{
	void *map;
	int resid;
	...
	map = &pas->Mapped;
	cc = drm_map_resource(mcon->PciHandle, resid + PCI_RESID_BAR0,
			      map);
	...
	if (cc) {					   /* mapping error */
		...
		SK_ERROR
		    ("PCI mapping failed while mapping BAR%d for module#%d",
...
static void *pci_module_hunt(SkelDrvrModuleContext * mcon)
{
	...
	i = 0;
	while (1) {
		cc = drm_get_handle(PCI_BUSLAYER, pma->VendorId,
				    pma->DeviceId, &handles[i]);
		if (cc) {
			handles[i] = NULL;
			goto out;
		}
...
static int AddPciModule(SkelDrvrModuleContext * mcon)
{
	...
	if (pcima) {
		...
		while (pcias) {
			if (!map_pcias(mcon, pcias)
			    && !mcon->Modld->IgnoreErrors)
				goto out_err;
			pcias = pcias->Next;
		}
	}
	...
	if (intrd && intrd->IsrFlag) {
		...
		cc = drm_register_isr(mcon->PciHandle, __pci_isr,
				      (void *) mcon);
