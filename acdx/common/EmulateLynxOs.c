...
/* ========================================================================== */
/* Here I emulate QUEUES LynxOs semaphores using Linux wait_queues.           */

#define QUEUES 16

static wait_queue_head_t  queues[QUEUES];
static int                qcounts[QUEUES];
static int               *lynx_sems[QUEUES];

/* ========================================================================== */
/* Convert a LynxOs semaphore into a Linux wait queue                         */

static wait_queue_head_t *get_wait_queue(int *sem, int *index) {
int i;
...
int indx, qc, cc;
   ...
   if (debug & LynxOsDEBUG_MODULE)
      printk(KERN_INFO "Debug: lynx_os: swait: sem: 0x%lx flag: %d\n",(unsigned long) sem,flag);

   wq = get_wait_queue(sem,&indx);
   if (wq) {
      qc = --qcounts[indx]; cc = 0;
      if (qc < 0) {
	 if (flag == SEM_SIGIGNORE)      wait_event(*wq, qcounts[indx] >= 0);
	 else                       cc = wait_event_interruptible(*wq, qcounts[indx] >= 0);
	 if (cc) {
	    if (debug & LynxOsDEBUG_ERRORS)
	       printk(KERN_WARNING "lynx_os: swait: Interrupted: %d\n",cc);
	    return SYSERR;
	 }
      }
      return OK;
   ...
   if (debug & LynxOsDEBUG_ERRORS)
      printk(KERN_WARNING "lynx_os: swait: EFAULT: No more wait queues\n");
   return SYSERR;
}

/* ============================================================================ */
/* Emulate the LynxOs call ssignal                                              */
...
int indx, qc;
   ...
   if (debug & LynxOsDEBUG_MODULE)
      printk(KERN_INFO "Debug: lynx_os: ssignal: sem: 0x%lx\n",(unsigned long) sem);
   ...
   if (debug & LynxOsDEBUG_ERRORS)
      printk(KERN_WARNING "lynx_os: ssignal: EFAULT: No more wait queues\n");
   return SYSERR;
}

/* ============================================================================ */
/* Emulate LynxOs call sreset                                                   */
...
int indx;
   ...
   if (debug & LynxOsDEBUG_MODULE)
      printk(KERN_INFO "Debug: lynx_os: sreset: sem: 0x%lx\n",(unsigned long) sem);
   ...
   if (debug & LynxOsDEBUG_ERRORS)
      printk(KERN_WARNING "lynx_os: sreset: EFAULT: No more wait queues\n");
   return SYSERR;
}
...
static DEFINE_MUTEX(acdx_drvr_alloc);
