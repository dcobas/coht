#!/usr/bin/perl
# Install modpci modules from a transfer.ref file and crateconfig file. 
...
close(INPUT);
  ...
  ENTRY: foreach my $href (@{$AoHref}) {
	...
	my $insmod = "insmod $driver_name lun=$index_parm bus=$bus_number slot=$slot_number vendor_id=0x1498 device_id=0x30C8 subvendor_id=0x1498 subdevice_id=0x300A";
	#print $insmod, "\n";
...
vmod_install.pl - Install the TPCI200 kernel module using crateconfig file.

=head1 SYNOPSIS

vmod_install.pl [OPTIONS] driver

vmod_install.pl - Install the TPCI200 kernel module using B<crateconfig> file.
...
=item B<--crateconfig>=FILE

Filename of a crateconfig alternative to the default '/etc/crateconfig'.
...
B<vmod_install.pl> parses a B<transfer.ref> file (taken by default from '/etc/crateconfig')
...
By default all of B<TPCI200> devices in the transfer.ref are processed.
