...
cvorb_t *cvorb_open(unsigned int lun)
{
    ...
    if (is_sysfs_fcn_enabled == -1) { /* not yet initialized */
        len = snprintf(path, CVORB_PATH_SIZE, "%s/%s", dev->sysfs_path, "sysfs_fcn_enabled");
        path[len] = '\0';
...
int cvorb_ch_set_fcn( cvorb_t *dev, unsigned int chnr, unsigned int fcnnr,
    ...
    {
        len = snprintf(attr_path, CVORB_PATH_SIZE, "%s/submodule.%d/channel.%d/fcn_%d",
                        dev->sysfs_path, fcn.submodulenr, fcn.channr, fcn.fcnnr);
        attr_path[len] = '\0';
        ret = cvorbdev_set_attr_bin(attr_path, (void *)fcn.hw_fcn, sz32*sizeof(uint32_t));
        if (ret < 0) {
            __cvorb_libc_error(__func__);
            res = -1;
        }
...
int cvorb_ch_get_fcn( cvorb_t *dev, unsigned int chnr, unsigned int fcnnr, 
            struct cvorb_vector_fcn *buff, unsigned int n_vector, unsigned int *fcn_sz) {
    struct cvorb_hw_fcn fcn;
...
//        sz32 = get_hw_fcn_size32(n_vtr);
    ...
    if(is_sysfs_fcn_enabled) /* via sysfs: default */
    {
        len = snprintf(attr_path, CVORB_PATH_SIZE, "%s/submodule.%d/channel.%d/fcn_%d",
                        dev->sysfs_path, fcn.submodulenr, fcn.channr, fcn.fcnnr);
        attr_path[len] = '\0';
        res = cvorbdev_get_attr_bin(attr_path, (void *)fcn.hw_fcn, sz32*sizeof(uint32_t));
        if (res < 0) {
             __cvorb_libc_error(__func__);
            goto error;
        }
