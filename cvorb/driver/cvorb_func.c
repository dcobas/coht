...
static int __cvorb_sram_busy(struct cvorb_submodule *submodule)
{
	...
	for (i = 0; i < 10; i++) {
		status =
		    cvorb_readw(submodule->parent,
				submodule->reg_offset +
				CVORB_SUBMODULE_STATUS);
...
{
	cvorb_writew(submodule->parent,
		     submodule->reg_offset + CVORB_SUBMODULE_CTL,
 ...
 */
int cvorb_submodule_trigger(struct cvorb_submodule *submodule,
			    enum cvorb_trigger event)
{
	uint32_t val;
	switch (event) {
	case CVORB_START:
	...
	cvorb_writew(submodule->parent,
		     submodule->reg_offset + CVORB_SUBMODULE_CTL, val);
	/* Leave critical section */
	/**************************/
	mutex_unlock(&submodule->parent->lock);
...
{
	return cvorb_readw(submodule->parent,
			   submodule->reg_offset +
			   CVORB_SUBMODULE_STATUS) & CVORB_SUBMODULE_BUSY;
}

/*
 * Submodule set input pulse polarity sysfs callback
 */
int cvorb_submodule_inpol(struct cvorb_submodule *submodule,
			  uint32_t value)
{
	...
	cvorb_writebitfields(submodule->parent,
			     submodule->reg_offset +
			     CVORB_SUBMODULE_CONFIG,
...
void cvorb_submodule_status(struct cvorb_submodule *submodule,
			    uint32_t * status)
{
	*status =
	    cvorb_readw(submodule->parent,
			submodule->reg_offset + CVORB_SUBMODULE_STATUS);
}

void cvorb_submodule_dacsource(struct cvorb_submodule *submodule,
			       uint32_t value)
{
	uint32_t hw_val = value;
	/* Enter Critical section */
...
void cvorb_submodule_ledsource(struct cvorb_submodule *submodule,
			       uint32_t value)
{
	uint32_t hw_val = value;
	/* Enter Critical section */
...
void cvorb_submodule_opticalsource(struct cvorb_submodule *submodule,
				   uint32_t value)
{
	uint32_t hw_val = value;
	/* Enter Critical section */
...
void cvorb_submodule_enable_opticalout(struct cvorb_submodule *submodule,
				       uint32_t value)
{
	uint32_t hw_val = value;
	/* Enter Critical section */
...
{
	*status =
	    cvorb_readw(channel->parent->parent,
			channel->reg_offset + CVORB_CH_STATUS);
	return 0;
}
...
int cvorb_ch_enable(struct cvorb_channel *ch, uint32_t value)
{
	...
	cvorb_writebitfields(ch->parent->parent,
			     ch->reg_offset + CVORB_CH_CONFIG,
...
int cvorb_select_fcn(struct cvorb_channel *ch, uint32_t value)
{
	...
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_SELECT_FCN, value);
	/* Leave critical section */
	/**************************/
	mutex_unlock(&ch->parent->parent->lock);
...
int cvorb_enable_fcn(struct cvorb_channel *ch, uint32_t value)
{
	...
	if (value < 32) {	/* Low register */
		cvorb_writew(ch->parent->parent,
			     ch->reg_offset + CVORB_CH_ENABLE_FCN_MASK_L,
			     (uint32_t) ch->enable_fcn_mask);
	} else {		/* High register */

		cvorb_writew(ch->parent->parent,
			     ch->reg_offset + CVORB_CH_ENABLE_FCN_MASK_H,
			     (uint32_t) (ch->enable_fcn_mask >> 32));
	}
	/* Leave critical section */
	/**************************/
	mutex_unlock(&ch->parent->parent->lock);
...
int cvorb_ch_repeat_count(struct cvorb_channel *ch, uint32_t value)
{
	...
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_REPEAT_FCN, value);
	/* Leave critical section */
	/**************************/
	mutex_unlock(&ch->parent->parent->lock);
...
static int __cvorb_set_fcn(struct cvorb_dev *cvorb,
			   struct cvorb_hw_fcn *fcn, int is_ioctl)
{
	uint32_t *hw_fcn;
	int ret = 0, i;
	...
	if (is_ioctl) {
		...
		hw_fcn =
		    kzalloc((ssize_t) fcn->hw_fcn_size32 *
		...
		if (copy_from_user
		    (hw_fcn, fcn->hw_fcn,
		     fcn->hw_fcn_size32 * sizeof(uint32_t))) {
			ret = -EFAULT;
	...
	if (get_endian() == LITTLE_ENDIAN) {
		...
		for (i = 0, swapp = hw_fcn; i < fcn->hw_fcn_size32;
		     ++i, ++swapp)
			*swapp = swab32(*swapp);
	}
#endif
	...
	cvorb_writew(cvorb,
		     cvorb->submodules[fcn->submodulenr].reg_offset +
		     CVORB_SUBMODULE_SRAM_ADDR, ramaddr);

	/* write the function itself */
	reg_offset =
	    cvorb->submodules[fcn->submodulenr].reg_offset +
	    CVORB_SUBMODULE_SRAM_DATA;
#ifdef __CVORB_DMA__
	ret =
	    cvorb_dma_write_mblt(cvorb->dev,
				 (int) cvorb->iomap + reg_offset, hw_fcn,
				 fcn->hw_fcn_size32 * sizeof(uint32_t));
#else
...
static int __cvorb_get_fcn(struct cvorb_dev *cvorb,
			   struct cvorb_hw_fcn *fcn, ssize_t * count,
			   int is_ioctl)
{
	...
	if (is_ioctl) {
		hw_fcn =
		    kzalloc((ssize_t) hw_fcn_sz32 * sizeof(uint32_t),
			    GFP_KERNEL);
		if (hw_fcn == NULL) {
...
out_free_lock:
	...
	if (is_ioctl && hw_fcn != NULL) {	/* buffer has been allocated */
		if (copy_to_user
		    (fcn->hw_fcn, (char *) hw_fcn,
		     hw_fcn_sz32 * sizeof(uint32_t)))
...
static int cvorb_ioctl_get_fcn(struct cvorb_dev *cvorb, void __user * arg)
{
	...
	if (copy_to_user
	    ((char *) arg, (char *) &fcn, sizeof(struct cvorb_hw_fcn)))
		return -EFAULT;	/* Bad address */
	return ret;
}
...
{
	cvorb_writew(ch->parent->parent, ch->reg_offset + CVORB_CH_CONFIG,
		     0);
	ch->enable = 0;
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_ENABLE_FCN_MASK_L, 0);
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_ENABLE_FCN_MASK_H, 0);
	ch->enable_fcn_mask = 0;
	/* For the repeat-count the default value is 1- 0 means infinite repeat count */
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_REPEAT_FCN, 1);
	ch->repeat_count = 1;
	cvorb_writew(ch->parent->parent,
		     ch->reg_offset + CVORB_CH_SELECT_FCN, 0);
	ch->selected_fcn = 0;
}
...
static void cvorb_init_submodule(struct cvorb_submodule *sm)
{
	...
	cvorb_writew(sm->parent, sm->reg_offset + CVORB_SUBMODULE_CONFIG,
		     0);
	sm->inpol = 0;
	sm->dac_source = 0;
	sm->led_source = 0;
...
static unsigned long cvorb_map(unsigned long base_address)
{
	return find_controller(base_address, CVORB_WINDOW_LENGTH,
			       CVORB_ADDRESS_MODIFIER, 0, CVORB_DATA_SIZE,
			       &param);
}
...
static int cvorb_open(struct inode *inode, struct file *file)
{
	struct cvorb_dev *cvorb =
	    container_of(inode->i_cdev, struct cvorb_dev, cdev);
		  ...
		  dev_t cvorb_devno, struct cvorb_dev *cvorb)
{
	...
	for (submoduleIdx = 0; submoduleIdx < CVORB_SUBMODULES;
		...
		submodule->reg_offset =
		    CVORB_SUBMODULE_OFFSET * submoduleIdx;
		...
		for (chanIdx = 0; chanIdx < CVORB_CHANNELS; ++chanIdx) {
			...
			channel->reg_offset =
			    submodule->reg_offset +
			    CVORB_CH_SIZE * chanIdx;
		}
	}

	if (cvorb->iomap == NULL) {
		printk(KERN_ERR PFX
		       "Can't find virtual address of the module's registers\n");
		ret = -EINVAL;
		goto out_free;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)
	cvorb->dev =
	    device_create(cvorb_class, parentDev, devno, NULL, "cvorb.%d",
			  cvorb->lun);
#else
	cvorb->dev =
	    device_create(cvorb_class, parentDev, devno, "cvorb.%d",
			  cvorb->lun);
#endif
 ...
 */
void cvorb_uninstall(struct class *class, struct cvorb_dev *cvorb,
		     dev_t cvorb_devno)
{
	dev_t devno = MKDEV(MAJOR(cvorb_devno), cvorb->lun);
	...
	/* Unmap the memory */
	return_controller((unsigned long) cvorb->iomap,
			  CVORB_WINDOW_LENGTH);
