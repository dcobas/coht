...
static int __cvorb_set_fcn(struct cvorb_dev *cvorb, struct cvorb_hw_fcn *fcn)
{
        uint32_t* hw_fcn;
        int ret = 0, i;
        ...
        /* In case of read/write 32 bits register endianess is done by ioread32be/iowrite32be routines */
        /* only if necessary, that is to say the CPU is little endian (knowing VME bus is big endian)  */
        ...
        ramaddr = ((fcn->channr)<<CVORB_SUBMODULE_SRAM_CHAN_SHIFT) | (fcn->fcnnr<<CVORB_SUBMODULE_SRAM_FUNC_SHIFT);
        ...
        cvorb_writew(cvorb, cvorb->submodules[fcn->submodulenr].reg_offset + CVORB_SUBMODULE_SRAM_ADDR, ramaddr);

        /* write the function itself */
        reg_offset = cvorb->submodules[fcn->submodulenr].reg_offset + CVORB_SUBMODULE_SRAM_DATA;
#ifdef __CVORB_DMA__
        ret = cvorb_dma_write_mblt(cvorb->dev, (int)cvorb->iomap+reg_offset, hw_fcn, fcn->hw_fcn_size32*sizeof(uint32_t));
#else
...
int cvorb_sysfs_set_fcn(struct cvorb_channel *channel, void *buffer)
{
        struct cvorb_hw_fcn *fcn = (struct cvorb_hw_fcn *)buffer;
        return __cvorb_set_fcn(channel->parent->parent, fcn);
}
...
static int cvorb_ioctl_set_fcn(struct cvorb_dev *cvorb, void __user *arg)
{
        ...
        if (copy_from_user(&fcn, arg, sizeof(fcn)))
                return -EFAULT;
        return (__cvorb_set_fcn(cvorb, &fcn));
}
...
out_free_lock:
        ...
        if (hw_fcn != NULL) { /* buffer has been allocated */
 ...
 */
int cvorb_install(struct class *cvorb_class, struct device *parentDev, dev_t cvorb_devno, struct cvorb_dev *cvorb)
{
...
/* This block has been moved at the end because potentially after calling cdev_add
the device becomes visible and can be called by the kernel. Therefore it should be fully initialized   
