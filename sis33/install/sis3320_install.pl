#!/usr/bin/perl
#
# sis3320_install.pl - install a sis3320 device
#
# documentation:
# sis3320_install.pl --help or sis3320_install.pl --man
#
...
if (@AoH) {
    ...
    die "No SIS3320 modules found in $transfer_ref.\n";
}

if (@AoH) {
    if (!module_is_loaded('sis3320')) {
	sis3320_install(\@AoH);
    }
} else {
    die "No SIS3320 modules found in $transfer_ref.\n";
...
sub sis3320_install {
  ...
  ENTRY: foreach my $href (@{$AoHref}) {
      next ENTRY if $href->{'module-type'} !~ m/SIS3320/;
    ...
    if (defined $index_parm) {
	my $insmod = "insmod sis3320.ko index=$index_parm base=$base_parm vector=$irq level=$level";
	system($insmod) == 0 or die("$insmod failed");
        print $insmod, "\n";
    }
}
...
sis3320_install.pl - Install the SIS3320 kernel module using transfer.ref file.

=head1 SYNOPSIS

sis3320_install.pl [OPTIONS]

sis3320_install.pl - Install the SIS3320 kernel module using B<transfer.ref> file.
...
B<sis3320_install.pl> parses a B<transfer.ref> file (taken by default from '/etc/transfer.ref')
...
are passed as arguments to the B<SIS3320> kernel module.
